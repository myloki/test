...
gulp.task("lessTask",function(){
    // * 匹配任意数量的字符
    gulp.src("./public/src/styles/less/*.less") //设置源文件的路径
        .pipe(sourceMaps.init()) //初始化资源地图
        .pipe(less({
            paths: [ path.join(__dirname, 'less', 'includes') ]
        })) //调用插件的方法去编译less文件
        .pipe(sourceMaps.write("./maps")) //写入资源地图
        .pipe(gulp.dest("./public/src/styles/css/")) //设置输出文件夹
...
gulp.task("cssClean",function () {
    // * 匹配任意数量的字符
    gulp.src("./public/src/styles/css/*.css") //设置源文件的路径
...
gulp.task("watch",function () {
    ...
    gulp.watch("./public/src/styles/less/**/*",["lessTask"]); //观察者，less+sass变化就执行编译,"sassTask"
	gulp.watch("./public/dist/css/**/*",["cssClean"]); //观察者，less+sass变化就执行编译,"sassTask"
});
